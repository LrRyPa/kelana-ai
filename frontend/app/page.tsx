...
export default function Home() {
    ...
    try {
        ...
        if (!createRes.ok) throw new Error("Gagal menyimpan data perjalanan.");
        ...
        setError(err.message || "Terjadi kesalahan saat memproses Rencana Perjalanan.");
    } finally {
        setLoading(false);
    }
    };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
        ...
        {/* Hero Section dengan Destination Image */}
        <section className="relative rounded-3xl overflow-hidden border border-slate-800 shadow-2xl bg-slate-900">
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-6 sm:p-10 max-w-3xl">
                <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 mb-3 w-fit">
                AI Travel Planner
                </span>
                ...
                Didukung oleh Amazon Bedrock AI untuk menyusun Rencana Perjalanan harian yang presisi, hemat budget, dan personal.
                </p>
            </div>
            </div>
        </section>
        ...
        {/* Input Form Section */}
        <section id="planner" className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            <span></span> Atur Parameter Perjalanan
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">
            {/* Form Fields: Stack Vertikal di Mobile, Grid 4 Kolom di Desktop */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {/* Destination */}
                ...
                <input
                    ...
                    className="w-full bg-slate-800/80 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                />
                </div>
            ...
            <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 disabled:opacity-50 text-white font-semibold py-4 px-6 rounded-xl shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
                {loading ? (
                <>
        ...
        {loading && (
            <section className="bg-slate-900/60 border border-indigo-500/30 rounded-2xl p-12 text-center space-y-4 animate-pulse">
            <div className="inline-block p-4 rounded-full bg-indigo-500/10 text-indigo-400">
                <span className="text-3xl animate-spin inline-block">⏳</span>
            </div>
            <h3 className="text-xl font-bold text-white">Sedang Menyusun Rencana Perjalanan...</h3>
        ...
        {error && (
            <section className="bg-rose-950/40 border border-rose-800 rounded-2xl p-6 text-center space-y-4">
            <div className="text-3xl">⚠️</div>
            <h3 className="text-lg font-bold text-rose-300">Gagal Memproses Rencana</h3>
            <p className="text-sm text-rose-400">{error}</p>
            <button
                onClick={handleSubmit}
                className="px-4 py-2 bg-rose-900/60 hover:bg-rose-800 text-rose-200 text-xs font-semibold rounded-lg border border-rose-700 transition-colors"
            >
                Coba Lagi
            </button>
            </section>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-8">
            ...
            <div className="flex items-center gap-3 text-sm bg-slate-800 px-4 py-2 rounded-xl border border-slate-700/60 w-fit">
            ...
            {trip.ai_itinerary
                ...
                .map((dayBlock, dayIdx) => {
                // 1. Bersihkan baris sampah, garis pembatas, dan baris kosong
                const validLines = dayBlock
                    .split("\n")
                    .map((l) => l.trim())
                    .filter((l) => {
                    if (!l || l === "---" || l === "***" || l === "___") return false;
                    // Buang baris yang tidak punya teks/huruf/angka (misal hanya titik atau bintang kosong)
                ...
                contentLines.forEach((line) => {
                    ...
                    // Normalisasi teks tanpa tanda bintang (*)
                ...
                // Filter out orphaned "activities_header" (LabelActivities yang tidak diikuti poin item)
                ...
                return (
                    ...
                    <div className="space-y-3 sm:pl-2">
                        ...
                        // Cek apakah poin ini berada di dalam daftar ber-bullet atau sub-item
